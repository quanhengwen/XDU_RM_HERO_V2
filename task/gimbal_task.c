...
/**
  * @brief          云台进入自瞄模式状态，角度的增量来源于视觉发送的del angle
  * @author         RM
  * @param[in]      yaw: yaw轴角度控制，为角度的增量 单位 rad
  * @param[in]      pitch: pitch轴角度控制，为角度的增量 单位 rad
  * @param[in]      gimbal_control_set:云台数据指针
...
//视觉自瞄坐标控制
static void gimbal_shoot_follow_limit();
...
void gimbal_task(void const *pvParameters)
{
   ...
   // }
    ...
    while (1)
    {
		...
		osDelayUntil(&gimbal_task_pre_tick,1);
        //vTaskDelay(GIMBAL_CONTROL_TIME);
...
static void gimbal_set_mode(gimbal_control_t *set_mode)
{
	...
	/****以下的代码从gimbal_behaviour_mode_set函数中拷贝而出*****/
	//set gimbal_behaviour variable
    //云台行为状态机设置
    gimbal_behavour_set(set_mode);
	
	if (gimbal_behaviour == SHOOT_FOLLOW)
    {
...
//云台行为控制函数gimbal_behaviour_control_set中的代码复制
...
static void gimbal_shoot_follow_limit()
{
	...
	static fp32 shoot_angle_yaw_set;
	static fp32 shoot_angle_pit_set;
	
	shoot_angle_yaw_set = gimbal_control.gimbal_yaw_motor.absolute_angle_set;
	shoot_angle_pit_set =gimbal_control.gimbal_pitch_motor.absolute_angle_set;
	
	gimbal_control.gimbal_yaw_motor.absolute_angle_set = rad_format(shoot_angle_yaw_set +  shoot_control.my_shoot_data.yaw_set);
	gimbal_control.gimbal_pitch_motor.absolute_angle_set = rad_format(shoot_angle_pit_set +  shoot_control.my_shoot_data.pit_set);

//	 if (gimbal_control.gimbal_yaw_motor == NULL)
//    {
//        return;
//    }
...
void gimbal_behaviour_control_set(fp32 *add_yaw, fp32 *add_pitch, gimbal_control_t *gimbal_control_set)
{
	...
	{
		gimbal_shoot_follw_control(add_yaw, add_pitch,  gimbal_control_set);		//自瞄状态下的跟随
	}
...
//&& !switch_is_down(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]) && !toe_is_error(DBUS_TOE)
  ...
  else
  {
    ...
    else if (switch_is_mid(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]) || (gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_F))
    {
		...
		//在进入底盘随动状态之前把云台进入初始化状态，进入初始化状态之后再进入底盘随动状态
...
static void gimbal_absolute_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
	...
	if(gimbal_control_set->gimbal_rc_ctrl->rc.s[0] == 1)
	*yaw = yaw_channel * (-0.0000012f) - gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN;
	else
