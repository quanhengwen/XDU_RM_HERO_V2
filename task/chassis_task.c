...
static void chassis_init(chassis_move_t *chassis_move_init)
{
    ...
    //max and min speed
    //最大 最小速度
    chassis_move_init->vx_max_speed = BattDirect_NORMAL_MAX_CHASSIS_SPEED_X;			//初始默认为电源供电下的底盘运动速度限幅
    chassis_move_init->vx_min_speed = -BattDirect_NORMAL_MAX_CHASSIS_SPEED_X;

    chassis_move_init->vy_max_speed = BattDirect_NORMAL_MAX_CHASSIS_SPEED_Y;
    chassis_move_init->vy_min_speed = -BattDirect_NORMAL_MAX_CHASSIS_SPEED_Y;
...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
	...
	if(switch_is_mid(chassis_move_control->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]) && (chassis_move_control->chassis_mode != CHASSIS_VECTOR_NO_FOLLOW_YAW))
    {
		...
		{
			chassis_move_control->vx_max_speed = BattDirect_NORMAL_MAX_CHASSIS_SPEED_X;
			chassis_move_control->vx_min_speed = -BattDirect_NORMAL_MAX_CHASSIS_SPEED_X;
			chassis_move_control->vy_max_speed = BattDirect_NORMAL_MAX_CHASSIS_SPEED_Y;
			chassis_move_control->vy_min_speed = -BattDirect_NORMAL_MAX_CHASSIS_SPEED_Y;
		}
		else if(CMS_Hub.Route_ref == CMS_PR_BuckBoost)
		{
			chassis_move_control->vx_max_speed = BuckBoost_NORMAL_MAX_CHASSIS_SPEED_X;
			chassis_move_control->vx_min_speed = -BuckBoost_NORMAL_MAX_CHASSIS_SPEED_X;
			chassis_move_control->vy_max_speed = BuckBoost_NORMAL_MAX_CHASSIS_SPEED_Y;
			chassis_move_control->vy_min_speed = -BuckBoost_NORMAL_MAX_CHASSIS_SPEED_Y;
		}
		//speed limit
        //速度限幅
...
//		{
//			//电源直供模式下转速慢
//			chassis_move_control->wz_set = CHASSIS_ROTATE_SPEED_LOW;
//			chassis_move_control->vx_max_speed = BattDirect_NORMAL_MAX_CHASSIS_SPEED_X;
//			chassis_move_control->vx_min_speed = -BattDirect_NORMAL_MAX_CHASSIS_SPEED_X;
//			chassis_move_control->vy_max_speed = BattDirect_NORMAL_MAX_CHASSIS_SPEED_Y;
//			chassis_move_control->vy_min_speed = -BattDirect_NORMAL_MAX_CHASSIS_SPEED_Y;
...
//		{
//			//超级电容供电模式下转速快
//			chassis_move_control->wz_set = CHASSIS_ROTATE_SPEED_HIGH;
//			chassis_move_control->vx_max_speed = BuckBoost_NORMAL_MAX_CHASSIS_SPEED_X;
//			chassis_move_control->vx_min_speed = -BuckBoost_NORMAL_MAX_CHASSIS_SPEED_X;
//			chassis_move_control->vy_max_speed = BuckBoost_NORMAL_MAX_CHASSIS_SPEED_Y;
//			chassis_move_control->vy_min_speed = -BuckBoost_NORMAL_MAX_CHASSIS_SPEED_Y;
...
//        chassis_move_control->vy_set = fp32_constrain(chassis_move_control->vy_set, chassis_move_control->vy_min_speed, chassis_move_control->vy_max_speed);
	...
	else if (switch_is_down(chassis_move_control->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]) || (chassis_move_control->chassis_mode == CHASSIS_VECTOR_NO_FOLLOW_YAW))
...
void CMS_routin_choose()
{
	...
	if(chassis_move.chassis_RC == NULL)
		return;
		
		
	//按下shift键之后打开超级电容供电，此时routin = 1
	//未按下shift键打开电池直供电，此时routin = 0
	//当routin = 3时是关闭超级电容的供电，当前版本不存在此功能

	if( CapChageVoltage >= 14.0f )
	{
		if(chassis_move.chassis_RC->key.v & KEY_PRESSED_OFFSET_SHIFT)
		{
			CMS_Hub.power_routin = CMS_PR_BuckBoost;		//选择超级电容供电
		}
		else
		{
			CMS_Hub.power_routin = CMS_PR_BattDirect;		//选择电池直供电
		}
	}
	else if(CapChageVoltage<14.0f)
	{
		CMS_Hub.power_routin = CMS_PR_BattDirect;			//选择电池直供电
