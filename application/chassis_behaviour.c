...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    //遥控器设置模式
    if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]) || (chassis_move_mode->chassis_RC->key.v & KEY_PRESSED_OFFSET_F))
    {
    ...
    else if (switch_is_down(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]) || (chassis_move_mode->chassis_RC->key.v & KEY_PRESSED_OFFSET_R))
    {
...
static void chassis_infantry_follow_gimbal_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    if (chassis_move_rc_to_vector->chassis_RC->key.v & SWING_KEY)
    {
        if (swing_flag == 0)
        {
            swing_flag = 1;
            swing_time = 0.0f;
			chassis_move_rc_to_vector->chassis_angle_pid.Kp = 60;
        }
    }
    else
    {
        swing_flag = 0;
		chassis_move_rc_to_vector->chassis_angle_pid.Kp = 24;
    }
