...
void USER_CMS_init(UART_HandleTypeDef *huart,CMS_Handler *CMS,DMA_HandleTypeDef *Tx,DMA_HandleTypeDef *Rx)
{
	...
	CMS->LimitCurrent = 1200;					//超级电容的最大充电电流
	//开始启动
//	USER_OpenPower();
//	CMS->Send_Flag = 0;
}
...
void APP_BatteryCombineBuckBoost2(void)
{
	...
	//回读电容电压滤波，但是感觉可以不用了
	// LowPassFilter_RC_1order(&CapChageVoltage,&CapChageVoltage,&CapChageVoltage_Past);

	//监测当前电容供电路径状态
	/***当超级电容供电时LED亮起 begin****/
	if(HoldingRegister[HR_CMS_PowerRouting]==CMS_PR_BuckBoost)
	{
		LED1_ON;
	}
	else
...
/****************发送控制指令和读取电容数据 begin *****************/
	if(CMS_Hub.Send_Flag)
	{
		if(CMS_Hub.SwitchCount%WAITINGGap==WAITINGGap1P)		//控制与modbus从机的通信频率
		{
			...
			{
				CMS_Hub.Next_Command = CommandList[0];			
				CMS_Hub.Next_Command();
				Error_Code = 1;						
				CMS_Hub.Send_Flag = 0;
			}
			else if( CapChageVoltage > 14.0f ) 		//只有电容电压大于14V时才能选择使用超级电容
			{
